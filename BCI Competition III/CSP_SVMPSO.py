...
def load_data(subject):
    ...
    fig.savefig(f"figures/sensors_{subject}.png")
    plt.close(fig)
...
def run_pipeline(subject):
    raw, y_all, pos_all = load_data(subject)
    visualize_signal(raw, subject)
    X_all, labels_all = create_epochs(raw, y_all, pos_all)

    is_train = ~np.isnan(labels_all)
    X = X_all[is_train]
    y = labels_all[is_train].astype(int)

    scaler = StandardScaler()
    X_flat = scaler.fit_transform(X.reshape(len(X), -1))
    X = X_flat.reshape(len(X), X.shape[1], X.shape[2])
