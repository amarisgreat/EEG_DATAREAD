...
from mne.decoding import FilterBankCSP
import numpy as np
import os
import re
...
def preprocess_eeg_file(file_path, output_dir):
    ...
    if len(np.unique(labels)) < 2:
        print(f"Not enough classes in {base_filename}. Skipping.")
        return
    ...
    X_psd_all = []
    for trial in data:
        psd, _ = psd_array_welch(trial, sfreq=sfreq, fmin=psd_fmin, fmax=psd_fmax,
    ...
    # FBCSP Features
    fbcsp_freqs = [(8, 12), (12, 16), (16, 20), (20, 24), (24, 30)]
    X_fbcsp_all = []
    for class_id in np.unique(labels):
        ...
        if len(np.unique(y_binary)) < 2:
            continue
        fbcsp = FilterBankCSP(n_components=4, filter_bank=fbcsp_freqs, log=True)
        X_fbcsp_class = fbcsp.fit_transform(data, y_binary)
        X_fbcsp_all.append(X_fbcsp_class)

    if not X_fbcsp_all:
        print(f"FBCSP failed for {base_filename}")
        return

    X_fbcsp_combined = np.concatenate(X_fbcsp_all, axis=1)

    # Combine all features
    X_combined = np.concatenate([X_psd_all, X_erd_combined, X_fbcsp_combined], axis=1)
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = "preprocessed_data_fbcsp"
